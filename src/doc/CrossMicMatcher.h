...
struct MatchSettings
{
    ...
    float refineMs = kMatchRefineMs;
    float weighting = kPlainWeighting;
...
struct MatchContext
{
    ...
    // Калибровка реально знает эту пару каналов. Такой prior — не грубая
    // подсказка для дальнего микрофона, а геометрия источника: сверка может
    // уточнить её GCC, но не должна заменить стабильную строку случайным
    // пиком огибающей в широком окне.
    std::array<bool, kMaxChannels> priorKnown {};
    std::array<double, kMaxChannels> priorSpread {};
};
...
// предсказывается как `t + d[j]`, приход берётся по огибающей самого канала
// рядом с предсказанием и уточняется субсэмплево взаимной корреляцией. Дальше
...
// допустимость лага.
//
// Почему приход по огибающей, а не широким поиском корреляции: на реальном
// просачивании широкий поиск даёт устойчивый и неверный ответ при любом
// отбеливании — коррелятор садится на период инструмента и держится за него
// с точностью до сотых миллисекунды (инвариант 5, docs/real-kit-protocol.md).
// Корреляция здесь уточняет предсказание в узком окне и ничего не ищет.
//

...
| Зона | Читать сначала |
|---|---|
| Быстрый вход второго разработчика | `docs/onboarding.md` |
| Где остановились, ветка, следующий шаг | `docs/status.md` |
| Протокол смены, что куда писать | `docs/handoff.md` |
| Продукт, этапы 1–5, алгоритмы | `docs/drum-editor-plan.md` |
...
Этапы 2–3 делаются в **standalone-редакторе**. Reaper round-trip и ARA —
delivery-каналы поверх той же модели, не отдельные реализации редактора.
...
`beat_doc` линкует `beat_dsp` и не линкует JUCE вовсе: RPP/ARA delivery
обязаны получиться адаптерами поверх него, а не второй такой же моделью.
...
9. **Не включать ARA, Celemony SDK, общий документ между инстансами** до
   delivery-этапа. Reaper RPP export раньше ARA допустим, но только как
   адаптер поверх sidecar/`beat_doc`, не как вторая модель состояния.

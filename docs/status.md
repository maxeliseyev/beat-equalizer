...
- На ветке: осциллограмма в строке канала, строки по материалу стенда,
  Solo/Mute, имена дорожек, кнопка Audio…, 0.6.0.

## Now

PR 13 открыт, ждёт приёмки. Ветка зелёная: `make test` 61 кейс,
`make test-gui` 33 кейса.
...
- Размер буфера задаётся только диалогом Audio… (обёртка Standalone), своего
  дефолта плагин не навязывает.
